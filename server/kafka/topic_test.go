...
func TestPrefixedTopic(t *testing.T) {
	...
	require.Equal(t, "user.events.registered", PrefixedTopic("user.events.registered"))

	t.Setenv("KAFKA_TOPIC_PREFIX", "dev.")
	require.Equal(t, "dev.user.events.registered", PrefixedTopic("user.events.registered"))
...
func TestLogicalTopic(t *testing.T) {
	t.Setenv("KAFKA_TOPIC_PREFIX", "dev.")
	require.Equal(t, "user.events.registered", LogicalTopic("dev.user.events.registered"))
	require.Equal(t, "other.topic", LogicalTopic("other.topic"))

	require.NoError(t, os.Unsetenv("KAFKA_TOPIC_PREFIX"))
	require.Equal(t, "dev.user.events.registered", LogicalTopic("dev.user.events.registered"))
}
